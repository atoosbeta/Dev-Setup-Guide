...
```
2. Tạo một migration (bảng) mới
```bash
npx sequelize-cli migration:generate --name <tên file>
```
3. Chạy migration cho một bảng cụ thể
```bash
npx sequelize-cli db:migrate --name <tên file>
```
4. Chạy migration để tạo bảng vào database
```bash
npx sequelize-cli db:migrate
```
5. Undo migration cho một bảng cụ thể (hoàn tác tạo bảng)
```bash
npx sequelize-cli db:migrate:undo --name <tên file>
```
6. Undo tất cả các migration (hoàn tác tất cả migration đã chạy)
```bash
npx sequelize-cli db:migrate:undo
```
7. Tạo một file seeder
```bash
npx sequelize-cli seed:generate --name <tên file>
```
8. Chạy một file seeder
```bash
npx sequelize-cli db:seed --seed <tên file>
```
9. Undo một file seeder
```bash
npx sequelize-cli db:seed:undo --seed <tên file>
```
10. Chạy tất cả file seeder
```bash
npx sequelize-cli db:seed:all
```
11. Undo tất cả file seeder (hoàn tác tất cả seeder đã chạy)
```bash
